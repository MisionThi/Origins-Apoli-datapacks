...
execute store result score @s thi_return run function thi_pocket_hand:detect_change/check with storage thi_endergui:storage Temp

# Run if a change is detected (function to call when change is detected, linked by thi_gui_target)
execute if score @s thi_return matches 1 run function #thi_pocket_hand:hand_changes
# execute if score @s thi_return matches 1 run say hand change
